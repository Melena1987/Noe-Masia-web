...
export const Campus: React.FC<SectionProps> = ({ id }) => {
  return (
    <section id={id} className="relative py-32 md:py-40 bg-fixed bg-cover bg-center bg-gray-900" 
      ...
      {/* Diagonal Cut Effect */}
      <div className="absolute top-0 left-0 w-full h-16 bg-brand-dark transform -skew-y-2 origin-top-left -translate-y-8 z-20"></div>

      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-16 flex flex-col items-center">
          
          {/* Logo Integration */}
          <div className="mb-8 w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-brand-orange/20 overflow-hidden shadow-2xl shadow-black/50 bg-white">
            <img 
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-16">
          {[
            ...
            <div key={idx} className="group bg-white/5 backdrop-blur-sm p-8 border border-white/10 hover:border-brand-orange/50 transition-colors duration-300">
              <item.icon className="w-10 h-10 text-brand-orange mb-6 group-hover:scale-110 transition-transform" strokeWidth={1.5} />
